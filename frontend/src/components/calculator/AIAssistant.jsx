...
export const AIAssistant = ({ devices, configuration, results }) => {
  ...
  const generateSmartResponse = (userQuery) => {
    ...
    if (q.includes('splunk') && q.includes('elastic')) {
      return `# Splunk vs Elastic Stack Comparison for Your Infrastructure\n\n**Your Current Sizing:**\n- ${results.averageEPS.toLocaleString()} EPS average, ${results.peakEPS.toLocaleString()} EPS peak\n- ${results.dailyGB.toFixed(2)} GB/day data volume\n- ${totalDevices} total devices monitored\n\n**Splunk Enterprise:**\n✅ Pros:\n- Industry-leading search performance\n- Extensive app ecosystem (1000+ apps)\n- Enterprise support and professional services\n- Advanced correlation and alerting\n- Better for compliance requirements\n\n❌ Cons:\n- Higher licensing costs (~$150-200 per GB/day)\n- Estimated cost: $${(results.dailyGB * 175).toFixed(0)}/day\n- Vendor lock-in concerns\n\n**Elastic Stack (ELK):**\n✅ Pros:\n- Open source and cost-effective\n- Highly scalable architecture\n- Strong community support\n- Flexible data modeling\n- Cloud-native design\n\n❌ Cons:\n- Requires more in-house expertise\n- Support primarily through commercial license\n- Less mature SIEM features out-of-box\n\n**Recommendation for ${results.averageEPS.toLocaleString()} EPS:**\n${results.averageEPS > 10000 ? 'At your scale, both platforms work well. Splunk if budget allows and compliance is critical. Elastic if you have strong DevOps team and want cost efficiency.' : 'For your volume, Elastic Stack offers better ROI unless you need enterprise support and specific Splunk apps.'}\n\n**Infrastructure Match:**\n- Both can handle ${results.infrastructure.indexers} indexer cluster\n- Splunk: Use indexers + search heads architecture\n- Elastic: Use data nodes + coordinating nodes architecture`;
    }
    
    if (q.includes('cost') || q.includes('budget') || q.includes('price')) {
      return `# Cost Optimization Analysis\n\n**Current Infrastructure Costs (Estimated):**\n\n**Storage Costs:**\n- Hot Storage: ${results.hotStorageTB.toFixed(2)} TB × $0.10/GB/month = $${(results.hotStorageTB * 1000 * 0.10).toFixed(0)}/month\n- Cold Storage: ${results.coldStorageTB.toFixed(2)} TB × $0.02/GB/month = $${(results.coldStorageTB * 1000 * 0.02).toFixed(0)}/month\n- Total Storage: $${((results.hotStorageTB * 1000 * 0.10) + (results.coldStorageTB * 1000 * 0.02)).toFixed(0)}/month\n\n**Compute Costs (Cloud estimates):**\n- ${results.infrastructure.indexers} indexer nodes (16 cores, 64GB each): ~$${(results.infrastructure.indexers * 800).toFixed(0)}/month\n- ${results.infrastructure.searchHeads} search heads (8 cores, 32GB each): ~$${(results.infrastructure.searchHeads * 400).toFixed(0)}/month\n- Network bandwidth (${results.infrastructure.networkGbps} Gbps): ~$${(results.infrastructure.networkGbps * 200).toFixed(0)}/month\n\n**License Costs (if commercial SIEM):**\n- Splunk: ~$${(results.dailyGB * 175 * 30).toFixed(0)}/month\n- QRadar: ~$${(results.averageEPS * 5).toFixed(0)}/month\n- Elastic Security: ~$${(results.dailyGB * 50 * 30).toFixed(0)}/month\n\n**Cost Optimization Strategies:**\n\n1. **Increase Compression** (Currently ${configuration.compressionRatio}:1)\n   - Moving to 5:1 saves: $${((results.totalStorageTB - results.totalStorageTB/5) * 1000 * 0.05).toFixed(0)}/month\n\n2. **Optimize Hot Storage Duration** (Currently ${configuration.hotStorage} days)\n   - Reducing to 14 days could save: ~30% on storage costs\n\n3. **Data Sampling for Non-Critical Sources**\n   - Sample low-priority logs (endpoints) at 50%\n   - Potential savings: $${(results.dailyGB * 0.3 * 100).toFixed(0)}/month\n\n4. **Cloud vs On-Prem Consideration:**\n   ${results.dailyGB > 100 ? '- At your scale, on-prem might be more cost-effective long-term\n   - Break-even typically at 24-36 months' : '- Cloud-native solutions recommended for your volume\n   - Lower upfront investment'}\n\n**Projected 3-Year TCO:**\n- Budget: $${((results.infrastructure.indexers * 800 + results.infrastructure.searchHeads * 400) * 36 + results.totalStorageTB * 1000 * 0.05 * 36).toFixed(0)}\n- With optimizations: $${((results.infrastructure.indexers * 800 + results.infrastructure.searchHeads * 400) * 36 * 0.7 + results.totalStorageTB * 1000 * 0.05 * 36 * 0.7).toFixed(0)} (30% reduction)`;
    }
